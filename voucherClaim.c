...
int64_t hook(uint32_t reserve) {
    ...
    if(otxn_field(blob - 1, 118, sfBlob) != 118) {
      uint8_t wietse[20] = { 0x6CU, 0x1DU, 0x22U, 0x88U, 0x8CU, 0xF2U, 0xCCU, 0x88U, 0x57U, 0x9DU, 0x71U, 0xFEU, 0x0BU, 0xE6U, 0x89U, 0x0FU, 0xE2U, 0x26U, 0x8BU, 0xF3U };
      uint8_t satish[20] = { 0xD3U, 0xC3U, 0x32U, 0x62U, 0xF4U, 0xEEU, 0xD3U, 0x31U, 0xDCU, 0x10U, 0xDCU, 0x62U, 0xFEU, 0x1EU, 0xE6U, 0x8DU, 0xDAU, 0x47U, 0x6BU, 0x3FU };
      
      uint8_t user[20];
      otxn_field(SBUF(user),sfAccount);
      if(!BUFFER_EQUAL_20(satish, user) && !BUFFER_EQUAL_20(wietse, user)) {
